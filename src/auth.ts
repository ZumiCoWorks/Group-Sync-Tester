  ...
  callbacks: {
    ...
    async jwt({ token, user, profile, account }) {
      ...
      if (account?.provider === 'azure-ad' && token.email) {
        ...
        if (account.refresh_token) {
          token.azureRefreshToken = account.refresh_token;
        }
        ...
        await saveAzureTokenRecord({
          email: token.email,
          accessToken: account.access_token ?? token.azureAccessToken ?? '',
          refreshToken: account.refresh_token ?? token.azureRefreshToken ?? '',
          expiresAt,
          tenantId: process.env.AZURE_AD_TENANT_ID ?? 'common',
        });
      }
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.role = token.role ?? (session.user.email?.toString().endsWith('@students.afda.co.za') ? 'student' : 'staff');
      }
