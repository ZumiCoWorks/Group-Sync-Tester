...
const NAV_ITEMS = [
  { href: '/worksuite', label: 'Overview' },
  { href: '/worksuite/ops', label: 'Ops' },
  { href: '/worksuite/tutor', label: 'Tutor' },
  { href: '/worksuite/student', label: 'Student' },
];
...
export function WorksuiteFrame({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_top_left,_rgba(255,127,80,0.18),_transparent_35%),radial-gradient(circle_at_top_right,_rgba(255,127,80,0.12),_transparent_28%),linear-gradient(180deg,_rgba(255,255,255,0.04),_transparent_20%)]" />
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-4 md:px-6 lg:px-8">
        <header className="sticky top-3 z-30 mb-6 rounded-3xl border border-white/10 bg-white/5 px-4 py-4 backdrop-blur-xl md:px-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-white/50">AFDA Worksuite</p>
                  <h1 className="text-xl font-black tracking-tight">Venues & Assessments</h1>
                </div>
              </div>
              <p className="mt-2 max-w-2xl text-sm text-white/65">{lockCopy}</p>
            </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <Badge variant="outline" className="rounded-full border-white/15 px-3 py-1 text-white/80">
              {user.displayName}
            </Badge>
            <Badge variant="outline" className="rounded-full border-white/15 px-3 py-1 text-white/80">
              {user.email}
            ...
            {canToggleRole && (
              <div className="flex items-center gap-2">
                <Button size="sm" variant="outline" className="rounded-full border-white/20 bg-transparent text-white hover:bg-white/10" onClick={() => setRole('staff')}>
                  Staff
                </Button>
                <Button size="sm" variant="outline" className="rounded-full border-white/20 bg-transparent text-white hover:bg-white/10" onClick={() => setRole('student')}>
                  Student
                </Button>
