...
import { getLockoutCopy, isScheduleLocked } from '@/worksuite/config';
import { useVenueBookingStore } from '../store/use-venue-booking-store';
import { VenueColumnKey } from '@/worksuite/types';
import { autoMapVenueColumns } from '@/worksuite/services/spreadsheet';
...
export function VenueBookingOverviewPage() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.12fr_0.88fr]">
      <Card className="border-border bg-card p-6 text-card-foreground shadow-2xl shadow-emerald-500/5 backdrop-blur-xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground">AFDA App / Operations</p>
            <h2 className="mt-2 text-3xl font-black">Venue Booking</h2>
            <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
              Import venue registers into the operations app and keep the occupancy timeline read-only.
            </p>
          </div>
          ...
          {preview && (
            <div className="rounded-2xl border border-border bg-secondary/40 p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {preview.sourceName} · {previewRows || preview.rows.length} data rows
                  </p>
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                ...
                <Button variant="outline" className="rounded-full border-border bg-background text-foreground hover:bg-secondary" onClick={() => store.resetToSeed()}>
      ...
      <div className="grid gap-4">
        <Card className="border-border bg-card p-5 text-card-foreground shadow-xl shadow-emerald-500/5 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            <MapPinned className="h-5 w-5 text-primary" />
            <div>
              <h3 className="text-lg font-bold">Venue Timeline</h3>
              <p className="text-sm text-muted-foreground">Read-only occupancy view for operations.</p>
            </div>
          </div>
        ...
        <Card className="border-border bg-card p-5 text-card-foreground shadow-xl shadow-emerald-500/5 backdrop-blur-xl">
          <p className="text-sm font-bold">Operations namespace</p>
          <p className="mt-2 text-sm text-muted-foreground">This app owns the venue import lane and keeps its own shell.</p>
        </Card>
      </div>
    </div>
  );
